...
from app.schemas import AudioInfo, EnhanceResult, LlmResult, Metrics, NoiseInfo, ProcessResult, RuntimeOpenAIConfig, TranscribeResult, Transcription
...
async def process_upload(file: UploadFile, runtime_config: RuntimeOpenAIConfig | None = None) -> ProcessResult:
    ...
    if runtime_config.asr_provider in {"local-whisper", "xunfei"} or runtime_config.has_asr_api_key or settings.openai_api_key:
        raw_text = "已跳过原始音频转写，以减少长音频处理时间。"
...
async def enhance_upload(file: UploadFile, runtime_config: RuntimeOpenAIConfig | None = None) -> EnhanceResult:
    ...
    enhance_speech_file(original_wav, enhanced_wav, atten_lim=runtime_config.atten_lim)
